...
function concatenador(archivos::Array, columnas::Array, nombre_archivo::String = "new_PuMAC.csv", faltantes::Bool = true, filas)
...
function concatenador(archivos::Array, columnas::Dict, filas::Dict, nombre_archivo::String = "new_PuMAC.csv", faltantes::Bool = true)
    ...
    for i in 1:size(archivos,1), j in 1:size(names(arreglo[i]),1), k in get(x, "A", 1)
        # Se obtienen solo las columnas deseadas por el usuario
        if (k == names(arreglo[i])[j])
            # Se asigna a x con los calores de la columna deseada por el usuario
            x = arreglo[i][:,k]
            ...
            for m in collect(keys(filas))
                if(k == m)
                    deleteat!(x, get(filas, m, 1)) 
                end
            end
